// bio data
export const bioData = {
  name: "Saikumar Veeraboina",
  profile: "/saikumar.jpg",
  roles: [
    "Full Stack Developer",
    "Frontend Developer",
    "Programmer",
    "Backend Developer",
  ],
  mobile: "7780601401",
  email: "veeraboinasai123@gmail.com",
  description:
    "Full-Stack (MERN) Developer passionate about building high-performance, scalable, and user-friendly web applications. Skilled in crafting intuitive interfaces and efficient back-end systems. Always learning and innovating to deliver cutting-edge digital solutions.",
  github: "https://github.com/Saikumarveeraboina",
  resume:
    "https://drive.google.com/file/d/1gHqS56Suj8EwzxOFMG220Q__N5qhCBpa/view?usp=sharing",
  linkedin:
    "https://www.linkedin.com/in/veeraboina-saikumar",
  instagram:
    "https://www.instagram.com/saikumar_1104?igsh=MXR1YnJteGY5NGg1cg%3D%3D",
};

//   skills
export const skills = [
  {
    title: "Frontend",
    skills: [
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },
      {
        name: "HTML",
        image: "/skills/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image: "/skills/css.png",
      },
      {
        name: "JavaScript",
        image: "/skills/JavaScript-logo.png",
      },
      {
        name: "Bootstrap",
        image: "/skills/bootstrap-logo-shadow.png",
      },
      {
        name: "Tailwind",
        image: "/skills/tailwind.png",
      },
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Node Js",
        image: "https://nodejs.org/static/images/logo.svg",
      },
      {
        name: "Express Js",
        image:
          "data:image/png;base64,",
      },
       {
         name: "Python",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg",
       },
      {
        name: "MongoDB",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
      },
      {
        name: "MySQL",
        image:
          "/skills/mysql.png",
      },
      {
        name: "Firebase",
        image: "https://www.vectorlogo.zone/logos/firebase/firebase-icon.svg",
      },
    ],
  },

  {
    title: "Others",
    skills: [
      {
        name: "Git",
        image: "/skills/git.png",
      },
      {
        name: "GitHub",
        image:
          "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
      },
      {
        name: "Netlify",
        image:
          "/skills/netlify.png",
      },
      {
        name: "VS Code",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
      },
      {
        name: "Vercel",
        image: "/skills/vercel.png",
      },
      {
        name: "Render",
        image: "/skills/render.png",
      },
      {
        name: "Postman",
        image: "/skills/postman.png",
      },
    ],
  },
];

// education
export const education = [
  {
    id: 0,
    img: "/education/pg.png",
    school: "Omega PG College - MCA, Hyderabad",
    date: "2024 - 2026",
    grade: "85%",
    desc: "I am currently pursuing a Master's degree in Commerce (MCA) with a specialization in Computer Applications. My coursework includes Web Development, Programming in C, Database Management Systems, Computer Networks, E-Commerce, Accounting, Financial Management, Business Law, Economics, and Taxation, among others.",
    sub: "Master of Computer Applications (MCA)",
  },
  {
    id: 1,
    img: "/education/degree.jpeg",
    school: "Kavitha Memorial Degree College, Khammam",
    date: "June 2021 - Jun 2024",
    grade: "67%",
    desc: "I completed my BSC course at Kavitha Memorial Degree College, specializing in  Mathematics, Computer Applications, Practical Training, and English.",
    sub: "Bachelor of Science (BSC) in Computer Applications",
  },
  {
    id: 2,
    img: "/education/inter.jpg",
    school: "Sri CV Raman Jr College, Khammam",
    date: "Sep 2019 - May 2021",
    grade: "93%",
    desc: "I completed my Intermediate course at Sri CV Raman Jr College, specializing in Mathematics, Physics, Chemistry, and English.",
    sub: "Intermediate (M.P.C)",
  },
  {
    id: 3,
    img: "/education/tenth.jpg",
    school: "TS Model School, Athmakur (S), Suryapet",
    date: "Jun 2018 - Mar 2019",
    grade: "8.8 CGPA",
    desc: "I completed my 10th-grade education at TS Model School, where I studied core subjects including Mathematics, Science, Social Studies, English, and a second language.",
    sub: "SSC(X)",
  },
];

// projects
export const projects = [
  {
    id: 1,
    title: "College Information Portal",
    date: "Jan 2026",
    description:
      "Built a full-fledged college information portal using React.js and Firebase Implemented user authentication features including Sign Up and Sign In using Firebase Authentication Developed modules for college map, notices, semester materials, and academic information",
    image: "/projects/omega.png",
    techstack: ["React.js", "Firebase", "Firestore,", "Authentication"],
    category: "frontend",
    github: "https://github.com/Saikumarveeraboina/Omega-Site",
    webapp: "https://omegapg-mca.vercel.app",
  },
  {
    id: 2,
    title: "AI-Powered Resume & Cover Letter Generator",
    date: "Sep 2025",
    description:
      "• Developed a web application using Gemini API to analyze job descriptions and user profiles, generating customized cover letters and ATS-optimized keywords.\n• Implemented skill-based content generation, experience-level adaptation, and responsive UI.",
    image: "/projects/airesume.png",
    techstack: [
      "React Js",
      "bootstrap",
      "Gemini API",
      "Netlify",
    ],
    category: "Frontend",
    github: "https://github.com/Saikumarveeraboina/resumeBuilderAI",
    webapp: "https://68f4a16615329bb7368cab6e--vermillion-biscotti-ea7a45.netlify.app",
  },
  
];

// course certifications
export const certification = [
  {
    id: 0,
    img: "/education/entri.jpeg",
    institute:
      "MERN Stack Developer | Trained at NxtWave CCBP 4.0 Technologies",
    duration: "2023 - 2024",
    desc: "I successfully completed the MERN Full Stack Development course at NxtWave CCBP 4.0 Technologies, where I gained hands-on experience in building full-stack applications using MongoDB, Express.js, React.js, and Node.js. During the course, I worked on real-world projects that enhanced my skills in front-end and back-end development, RESTful API creation, authentication, database management, and deployment.",
    certificate:
      "#",
  },
  // {
  //   id: 1,
  //   img: "/education/illinois.png",
  //   institute:
  //     "Full Stack Development (MERN Stack) – Illinois Institute of Technology",
  //   duration: "2023 - 2024",
  //   desc: "I successfully earned the Full Stack Development (MERN Stack) certification from the Illinois Institute of Technology, showcasing my expertise in MongoDB, Express.js, React.js, and Node.js. This certification reflects my ability to develop dynamic, scalable web applications, implement RESTful APIs, manage databases, and build user-friendly interfaces. It highlights my commitment to continuous learning and staying updated with modern web technologies.",
  //   certificate:
  //     "https://drive.google.com/file/d/1tDnlNmOU4SrX1zvJplV_-SUwAW0nTleH/view?usp=sharing",
  // },
  // {
  //   id: 2,
  //   img: "/education/entri.jpeg",
  //   institute: "Internship at Entri Elevate Software Pvt Ltd",
  //   duration: "Nov 2024",
  //   desc: "I completed a one-month internship at Entri Elevate, where I gained hands-on experience in full-stack development. During this period, I built a social media application and a movie booking application, applying my skills in MongoDB, Express.js, React.js, and Node.js. This internship strengthened my ability to develop scalable web applications, integrate APIs, and enhance user experience through modern UI/UX practices.",
  //   certificate:
  //     "https://drive.google.com/file/d/1MeFtGV6nMe1-uzj_mqsc8sCnXXlG7Wtl/view?usp=drive_link",
  // },
];
